{{-- resources/views/orders/order-progress.blade.php --}}
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>Order Detail • On-Progress</title>
  ...
  <section class="safe-area">
    ...
    <div class="section pt-3">
      @php
        ...
        $thumb = $item?->service?->game ? asset('assets/' . Str::slug($item->service->game->game_name) . '.jpg') : asset('assets/abyss.jpg');
      ...
      <span class="badge-chip badge-prog">{{ $statusName }}</span>
      ...
      <div class="card-soft booster-row">
        <img src="{{ asset('assets/pp.jpg') }}" class="avatar" alt="">
      ...
      <div class="card-soft">
        ...
        <ul class="mb-0 mt-1">
          <li>{{ $item?->service?->service_desc }}</li>
        </ul>
        @endif
      </div>
    ...
    <div class="action-bar">
      <div class="d-flex gap-2">
        ...
        <form method="post" action="{{ route('orders.update.status', $order->order_id) }}" class="w-50">
          @csrf
          <input type="hidden" name="status" value="Completed">
          <button type="submit" class="btn btn-blue btn-pill w-100">Mark Complete</button>
        </form>
      </div>
    </div>
  </section>
